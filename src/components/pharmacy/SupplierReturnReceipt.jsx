...
export default function SupplierReturnReceipt({
  returnData,
  hospitalSettings,
  onClose,
}) {
  ...
  const printReceipt = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Supplier Return Receipt</title>
          <style>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b p-4 flex justify-between items-center">
          <h2 className="text-lg font-semibold">Supplier Return Receipt</h2>
          <button
            onClick={onClose}
            ...
            >
              ...
              <div className="text-center mb-3">
                ...
                <div className="font-bold text-sm mb-2">
                  Supplier Return # {returnData.returnNumber}
                </div>
              </div>

              {/* Supplier Info */}
              <div className="mb-3 text-xs">
                <div>Supplier: {returnData.supplierName}</div>
                <div>Date: {formatDate(returnData.returnDate)}</div>
                {returnData.originalPurchaseOrderNo && (
                  <div>Original PO: {returnData.originalPurchaseOrderNo}</div>
                )}
              </div>
              ...
              <div className="text-center text-xs">
                <div className="mb-1">Return processed successfully</div>
