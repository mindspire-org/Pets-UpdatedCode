...
export default function ReturnHistory() {
  ...
  const [filters, setFilters] = useState({
    startDate: "", endDate: "", returnType: "", refundStatus: "", customerName: "",
  });
  ...
  const fetchReturns = async () => {
    try {
      ...
      const res = await pharmacyHistoryAPI.getReturns(params);
  ...
  const applyFilters = () => { setPagination(p => ({ ...p, currentPage: 1 })); setTimeout(fetchReturns, 0); };
  const clearFilters = () => {
    setFilters({ startDate: "", endDate: "", returnType: "", refundStatus: "", customerName: "" });
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  const typeColor = (t) => t === "Customer Return" ? "bg-blue-100 text-blue-800" : "bg-purple-100 text-purple-800";
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-3">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={fetchReturns} className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">
          ...
          <div className="border-t pt-3">
            ...
            <div className="mb-3">
              <DateRangePicker fromDate={filters.startDate} toDate={filters.endDate}
                onDateRangeChange={dr => setFilters(p => ({ ...p, startDate: dr.fromDate, endDate: dr.toDate }))} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
              <input type="text" placeholder="Customer / Supplier Name" value={filters.customerName}
                onChange={e => setFilters(p => ({ ...p, customerName: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              <select value={filters.returnType} onChange={e => setFilters(p => ({ ...p, returnType: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {["Return No", "Type", "Date", "Customer / Supplier", "Original Invoice", "Items", "Total Return", "Refund Method", "Actions"].map(h => (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  <tr><td colSpan="10" className="px-4 py-3 text-center text-gray-500 text-sm">Loading return history...</td></tr>
                ) : returns.length === 0 ? (
                  <tr><td colSpan="10" className="px-4 py-3 text-center text-gray-500 text-sm">No returns found</td></tr>
                ) : (
                  returns.map(ret => (
                    <tr key={ret._id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 whitespace-nowrap font-mono text-sm font-semibold text-purple-700">{ret.returnNumber}</td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${typeColor(ret.returnType)}`}>
                          {ret.returnType}
                        </span>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap text-xs text-gray-600">
                        {new Date(ret.returnDate || ret.createdAt).toLocaleDateString()}<br />
                        <span className="text-gray-400">{new Date(ret.returnDate || ret.createdAt).toLocaleTimeString()}</span>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{ret.customerName || ret.supplierName || "—"}</div>
                        <div className="text-xs text-gray-500">{ret.customerContact || ret.supplierContact || ""}</div>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap font-mono text-xs text-blue-700">{ret.originalInvoiceNumber || ret.originalPurchaseOrderNo || "—"}</td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <div className="text-xs text-gray-700">{(ret.items || []).map(i => i.medicineName).join(", ")}</div>
                        <div className="text-xs text-gray-400">{ret.items?.length || 0} item(s)</div>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap font-bold text-orange-700">
                        PKR {Number(ret.totalReturnAmount || 0).toFixed(2)}
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap text-xs text-gray-700">{ret.refundMethod}</td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <div className="flex gap-2 items-center">
                          <button onClick={() => { setDetailReturn(ret); setShowDetail(true); }}
                            className="text-blue-600 hover:text-blue-900" title="View Details">
                            <FiEye size={16} />
                          </button>
                          <button onClick={() => printReturnSlip(ret)}
                            className="text-purple-600 hover:text-purple-900" title="Print Slip">
                            <FiPrinter size={16} />
                          </button>
                        </div>
                      </td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
            <div className="bg-gradient-to-r from-purple-600 to-blue-600 px-6 py-4 flex items-center justify-between shrink-0">
              <div className="text-white">
                ...
                <p className="text-xs text-purple-200 mt-0.5">{detailReturn.returnType} · {new Date(detailReturn.returnDate || detailReturn.createdAt).toLocaleString()}</p>
              </div>
              <div className="flex gap-2">
                <button onClick={() => printReturnSlip(detailReturn)}
                ...
                <button onClick={() => setShowDetail(false)} className="text-white hover:bg-white/20 p-1.5 rounded-lg transition-all">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                {[
                  ...
                  { label: "Date", value: new Date(detailReturn.returnDate || detailReturn.createdAt).toLocaleString() },
                  { label: detailReturn.returnType === "Customer Return" ? "Customer" : "Supplier", value: detailReturn.customerName || detailReturn.supplierName || "—" },
                  { label: "Contact", value: detailReturn.customerContact || detailReturn.supplierContact || "—" },
                  { label: "Original Invoice", value: detailReturn.originalInvoiceNumber || "—" },
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        {["Medicine", "Batch", "Category", "Qty", "Unit Price", "Total", "Reason"].map(h => (
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {(detailReturn.items || []).map((item, i) => (
                        <tr key={i} className="hover:bg-slate-50">
                          ...
                          <td className="px-3 py-2.5 font-mono text-xs text-slate-500">{item.batchNo || "—"}</td>
                          <td className="px-3 py-2.5 text-xs">
                            <span className="px-2 py-0.5 rounded-full bg-purple-100 text-purple-700">{item.category || "—"}</span>
                          </td>
                          <td className="px-3 py-2.5 text-center font-medium text-slate-700">{item.quantity} {item.unit}</td>
                          <td className="px-3 py-2.5 text-slate-700">PKR {Number(item.returnPrice || 0).toFixed(2)}</td>
                          <td className="px-3 py-2.5 font-bold text-orange-700">PKR {Number(item.totalReturnAmount || 0).toFixed(2)}</td>
                          <td className="px-3 py-2.5 text-xs text-slate-500">{item.reason || "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="bg-orange-50 border-t-2 border-orange-200">
                      <tr>
                        <td colSpan={5} className="px-3 py-2.5 text-right font-bold text-slate-700">Total Return Amount:</td>
                        <td className="px-3 py-2.5 font-black text-orange-700 text-base">PKR {Number(detailReturn.totalReturnAmount || 0).toFixed(2)}</td>
                        <td />
                      </tr>
                    </tfoot>
                  </table>
