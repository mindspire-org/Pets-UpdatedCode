...
export default function SalesReturn() {
  ...
  useEffect(() => {
    if (!filters.startDate && !filters.endDate && !filters.customerName &&
        !filters.customerContact && !filters.invoiceNumber && !filters.paymentMethod && !filters.status) {
      fetchSales();
    }
  ...
  const fetchSales = async () => {
    try {
      ...
      const res = await pharmacyHistoryAPI.getSalesHistory(params);
  ...
  const applyFilters = () => { setPagination(p => ({ ...p, currentPage: 1 })); setTimeout(fetchSales, 0); };
  const clearFilters = () => {
    setFilters({ startDate: "", endDate: "", customerName: "", customerContact: "", invoiceNumber: "", paymentMethod: "", status: "" });
  ...
  // Open return dialog — pre-populate items with qty=0
  ...
  const returnTotal = returnItems.reduce((s, it) => {
    const unitPrice = it.originalQty > 0 ? it.totalPrice / it.originalQty : it.pricePerUnit;
  ...
  const handleSubmitReturn = async () => {
    ...
    try {
      ...
      const res = await pharmacyHistoryAPI.createCustomerReturn({
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  const exportToCSV = () => {
    ...
    const rows = sales.map(s => ({
      "Invoice": s.invoiceNumber, "Date": new Date(s.createdAt).toLocaleDateString(),
      "Customer": s.customerName || "Walk-in", "Contact": s.customerContact || "",
      "Total": s.totalAmount, "Payment": s.paymentMethod,
    }));
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-3">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={exportToCSV} className="flex items-center gap-2 px-3 py-1.5 bg-green-600 text-white rounded hover:bg-green-700 text-sm">
                <FiDownload size={14} /> Export CSV
              </button>
              <button onClick={fetchSales} className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 text-white rounded hover:bg-blue-700 text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
            {[
              { label: "Total Sales", value: summary.totalSales, color: "blue" },
              { label: "Total Revenue", value: `PKR ${summary.totalRevenue?.toLocaleString()}`, color: "green" },
              { label: "Total Discount", value: `PKR ${summary.totalDiscount?.toLocaleString()}`, color: "orange" },
              { label: "Avg Sale Amount", value: `PKR ${summary.avgSaleAmount?.toFixed(0)}`, color: "purple" },
          ...
          <div className="border-t pt-3">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium text-gray-700">Optional Filters</span>
            </div>
            <div className="mb-3">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {["Invoice Details", "Customer Info", "Medicines", "Qty (each)", "Qty", "Amount", "Payment", "Sold By", "Actions"].map(h => (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  <tr><td colSpan="9" className="px-4 py-3 text-center text-gray-500 text-sm">Loading sales...</td></tr>
                ) : sales.length === 0 ? (
                  <tr><td colSpan="9" className="px-4 py-3 text-center text-gray-500 text-sm">No sales found</td></tr>
                ) : (
                  sales.map(sale => (
                    <tr key={sale._id} className="hover:bg-gray-50">
                      <td className="px-4 py-2 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{sale.invoiceNumber}</div>
                        <div className="text-xs text-gray-500">{new Date(sale.createdAt).toLocaleDateString()} {new Date(sale.createdAt).toLocaleTimeString()}</div>
                        <div className="text-xs text-gray-400">{sale.items?.length || 0} items</div>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{sale.customerName || "Walk-in Customer"}</div>
                        <div className="text-xs text-gray-500">{sale.customerContact}</div>
                      </td>
                      <td className="px-4 py-2 max-w-[160px]">
                        <div className="text-xs text-gray-800 leading-relaxed">{(sale.items || []).map(i => i.medicineName).join(", ")}</div>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <div className="text-xs text-gray-600">{(sale.items || []).map(i => i.quantity).join(", ")}</div>
                      </td>
                      <td className="px-4 py-2 text-center whitespace-nowrap">
                        <span className="text-sm font-bold text-gray-800">{(sale.items || []).reduce((s, i) => s + (Number(i.quantity) || 0), 0)}</span>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <div className="text-sm font-bold text-gray-900">PKR {Number(sale.totalAmount || 0).toFixed(2)}</div>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{sale.paymentMethod}</span>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
                        <span className="text-xs font-medium text-purple-700 bg-purple-50 px-2 py-1 rounded-full">{sale.soldBy || loggedInUser}</span>
                      </td>
                      <td className="px-4 py-2 whitespace-nowrap">
            ...
            <div className="bg-gradient-to-r from-orange-500 to-red-500 px-6 py-4 flex items-center justify-between shrink-0">
              <div className="text-white">
                ...
                <p className="text-xs text-orange-100 mt-0.5">
                  {returnSale.invoiceNumber} · {returnSale.customerName || "Walk-in"} · {new Date(returnSale.createdAt).toLocaleDateString()}
                </p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div className="bg-slate-50 rounded-lg px-3 py-2">
                  ...
                  <p className="font-bold text-slate-800 font-mono">{returnSale.invoiceNumber}</p>
                </div>
                <div className="bg-slate-50 rounded-lg px-3 py-2">
                  <p className="text-xs text-slate-500">Total Amount</p>
                  <p className="font-bold text-slate-800">PKR {Number(returnSale.totalAmount || 0).toFixed(2)}</p>
                </div>
                <div className="bg-slate-50 rounded-lg px-3 py-2">
                  <p className="text-xs text-slate-500">Payment</p>
                  <p className="font-bold text-slate-800">{returnSale.paymentMethod}</p>
                </div>
              </div>
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        {["Medicine", "Batch", "Type", "Sold Qty", "Unit Price", "Return Qty", "Return Amt"].map(h => (
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {returnItems.map((item, idx) => {
                        const unitPrice = item.originalQty > 0 ? item.totalPrice / item.originalQty : item.pricePerUnit;
                        ...
                        return (
                          <tr key={idx} className={item.returnQty > 0 ? "bg-orange-50" : "hover:bg-slate-50"}>
                            ...
                            <td className="px-3 py-2.5 text-xs text-slate-600">
                              <span className={`px-2 py-0.5 rounded-full font-medium ${item.sellBy?.toLowerCase() === 'pack' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                                {item.sellBy?.toLowerCase() === 'pack' ? 'Pack' : 'Loose'}
                              </span>
                            </td>
                            <td className="px-3 py-2.5 text-center font-medium text-slate-700">{item.originalQty}</td>
                            <td className="px-3 py-2.5 text-slate-700">PKR {unitPrice.toFixed(2)}</td>
                    ...
                    {returnTotal > 0 && (
                      <tfoot className="bg-orange-50 border-t-2 border-orange-200">
                        <tr>
                          <td colSpan={5} className="px-3 py-2.5 text-right font-bold text-slate-700">Total Refund:</td>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="button" onClick={handleSubmitReturn}
      ...
      {showSlip && slipData && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            {/* Slip body — mimics the thermal receipt */}
            <div className="p-6 font-mono text-sm">
              <div className="text-center font-bold text-lg tracking-wide mb-1">
                {hospitalSettings?.companyName?.toUpperCase() || "PHARMACY"}
              </div>
              <div className="border-t border-dashed border-slate-400 my-2" />
              <div className="text-center font-semibold text-base mb-2">Customer Return</div>
              <div className="border-t border-dashed border-slate-400 my-2" />
                ...
                <tbody>
                  {slipData.items.map((it, i) => (
                    <tr key={i}>
                      ...
                      <td className="text-center py-1">{it.sellBy?.toLowerCase() === 'pack' ? 'Pack' : 'Loose'}</td>
                      <td className="text-center py-1">{it.returnQty}</td>
                      <td className="text-right py-1">{Number(it.returnAmt).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="border-t border-dashed border-slate-400">
                    <td colSpan={2} className="pt-2 font-bold text-sm">TOTAL RETURN</td>
                    <td className="text-right pt-2 font-bold text-sm">Rs {Number(slipData.totalReturnAmount).toFixed(2)}</td>
                  </tr>
                </tfoot>
              </table>
