...
export default function SupplierReturns() {
  ...
  const returnTotal = returnItems.reduce(
    (s, it) => s + it.purchasePrice * it.returnQty,
    0,
  );
  ...
  const handleSubmitReturn = async () => {
    ...
    try {
      ...
      const res = await petshopPharmacyHistoryAPI.createSupplierReturn({
        ...
        processedBy:
          JSON.parse(localStorage.getItem("shop_auth") || "{}").name ||
          "Shop",
      });
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3">
            {[
              ...
              {
                label: "Total Spent",
                value: `PKR ${summary.totalAmount?.toLocaleString()}`,
                color: "green",
              },
              {
                label: "Total Paid",
              ...
              {
                label: "Outstanding",
                value: `PKR ${summary.outstandingAmount?.toLocaleString()}`,
                color: "red",
              },
                ...
                <tr>
                  {[
                    ...
                    { label: "Stock", tooltip: null },
                    {
                      ...
                      tooltip: "Original amount / After returns",
                    },
                      ...
                      <td className="px-4 py-2">
                        <div className="text-xs font-bold text-blue-600 leading-relaxed">
                          {(purchase.items || [])
                            .map((i) => {
                              const stock =
                                i.currentStock || i.totalItems || i.quantity || 0;
                              return `${stock}`;
                            })
                            .join(", ")}
                        </div>
                          ...
                          {purchase.totalReturnedAmount > 0 ? (
                            <>
                              <span className="line-through text-gray-400 text-xs">
                                PKR {Number(
                                  purchase.netTotal ?? purchase.totalAmount ?? 0,
                                ).toFixed(2)}
                              </span>
                              <br />
                              <span className="text-green-600">
                                PKR {Number(purchase.remainingAmount ?? 0).toFixed(2)}
                              </span>
                            </>
                          ) : (
                            <span>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="bg-slate-50 rounded-lg px-3 py-2">
                  ...
                  <p className="font-bold text-slate-800">
                    PKR {Number(
                      returnPurchase.netTotal ?? returnPurchase.totalAmount ?? 0,
                    ).toFixed(2)}
                  </p>
                </div>
              </div>
                      ...
                      {returnItems.map((item, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-3 py-2.5 text-center font-medium text-blue-600 font-bold">
                              {item.currentStock}
                            </td>
                            <td className="px-3 py-2.5 text-slate-700">
                              PKR {item.purchasePrice.toFixed(2)}
                            </td>
                            <td className="px-3 py-2.5">
                              <input
                                ...
                                placeholder="0"
                                onChange={(e) =>
                                  updateReturnQty(idx, e.target.value)
                                }
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-8 font-mono text-sm max-h-[70vh] overflow-y-auto">
              <div className="text-center font-bold text-2xl mb-2 text-slate-900">
                SHOP
              </div>
              ...
              <table className="w-full text-sm">
                ...
                <tfoot>
                  <tr className="border-t-2 border-dashed border-slate-400">
                    <td
                      colSpan={2}
                      className="pt-4 font-black text-base text-slate-900"
                    >
                      TOTAL RETURN
                    </td>
                    <td className="text-right pt-4 font-black text-base text-slate-900">
              ...
              <div className="text-center text-slate-500 font-bold italic">
                Thank you!
              </div>
            </div>
          </div>
