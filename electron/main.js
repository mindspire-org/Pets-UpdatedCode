...
function resolveBackendEntry() {
  ...
  const candidates = [
    // 1. Unpacked extraResources - this is where we tell electron-builder to put it
    path.join(process.resourcesPath, 'server', 'server.js'),
    // 2. Resource path fallback
    path.join(app.getAppPath(), '..', 'server', 'server.js'),
    // 3. App Data path fallback
    path.join(app.getAppPath(), 'server', 'server.js'),
    // 4. Preferred: server unpacked alongside app.asar
    path.join(process.resourcesPath, 'app.asar.unpacked', 'server', 'server.js'),
    // 5. Packed inside app.asar (Fallback - may fail if dependencies are not inside)
  ...
  for (const c of candidates) {
    try {
      if (fs.existsSync(c)) {
        ...
        if (stats.isFile()) {
          console.log('[backend] Found entry at:', c);
          return c;
        }
      }
    } catch (err) {
      console.error(`[backend] Error checking path ${c}:`, err.message);
    }
  }
